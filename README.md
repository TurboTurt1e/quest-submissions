...
Standards can be beneficial to the Flow ecosystem so that users and smart contracts have a unified way of interacting with simmilar thing and simmilar but slightly different things. Standards can also establish pre-defined patterns to follow so that developers do not have to reinvent the wheel.


2) What is YOUR favourite food? Sushi
...
In the example presented in the lesson, "Force casting" downcasts the generic type "@NonFungibleToken.NFT" type to the more specific type "@NFT". We do this to make sure that the incoming token is compatible with our specific collection type which is "@NFT".

2) What does auth do? When do we use it?

The auth keyword is used for an "authorized reference".  We use the "authorized reference" and the "force cast" operator "as!" to "downcast" a generic type to a more specific type.
...
pub contract CryptoPoops: NonFungibleToken {
  ...
  // insert ICollectionPublic at the top and implement interface 
  pub resource Collection: NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, ICollectionPublic {
    ...
    // implement borrowAuthNFT functionality
    pub fun borrowAuthNFT(id: UInt64): &NFT {
        let authReference =  &self.ownedNFTs[id] as auth &NonFungibleToken.NFT
        return authReference as! &NFT
    }
...
You will have to write all the transactions to set up the accounts, 
```
...
transaction() {
  prepare(signer: AuthAccount) {
    ...
    signer.link<&CryptoPoops.Collection{NonFungibleToken.CollectionPublic, CryptoPoops.ICollectionPublic}>(/public/Collection, target: /storage/Collection)
  }
}
...
transaction(recipient: Address) {
  prepare(signer: AuthAccount) {
    let mintFactory = signer.borrow<&CryptoPoops.Minter>(from: /storage/Minter)?? panic("error")

    let publicCollectionRef = getAccount(recipient).getCapability(/public/Collection).borrow<&CryptoPoops.Collection{NonFungibleToken.CollectionPublic}>()?? panic("error")

    publicCollectionRef.deposit(token: <- mintFactory.createNFT(name: "Turbo", favouriteFood: "Sushi", luckyNumber: 0))
...
pub fun main(account: Address, id: UInt64) {
  let borrowedICollection = getAccount(account).getCapability(/public/Collection).borrow<&CryptoPoops.Collection{CryptoPoops.ICollectionPublic}>()?? panic("error")

  let borrowedNFT: &CryptoPoops.NFT = borrowedICollection.borrowAuthNFT(id: id)
  log(borrowedNFT.name)
  log(borrowedNFT.favouriteFood)
  log(borrowedNFT.luckyNumber)
